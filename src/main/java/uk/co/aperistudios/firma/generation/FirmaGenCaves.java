...
public class FirmaGenCaves extends MapGenBase {
    protected static final IBlockState BLK_LAVA = FirmaMod.lava.getBlock().getDefaultState();
            ...
            if (flag2 || random.nextInt(4) != 0)
            {
                ...
                if (p_180702_6_ >= d0 - 16.0D - d2 * 2.0D && p_180702_10_ >= d1 - 16.0D - d2 * 2.0D && p_180702_6_ <= d0 + 16.0D + d2 * 2.0D && p_180702_10_ <= d1 + 16.0D + d2 * 2.0D)
                {
                    ...
                    if (!flag3)
                    {
                        ...
                        {
                            double d10 = (j3 + p_180702_3_ * 16 + 0.5D - p_180702_6_) / d2;

                            for (int i2 = i3; i2 < i1; ++i2)
                            {
                                double d8 = (i2 + p_180702_4_ * 16 + 0.5D - p_180702_10_) / d2;
                                boolean flag1 = false;
                                ...
                                if (d10 * d10 + d8 * d8 < 1.0D)
                                {
                                    ...
                                    {
                                        double d9 = (j2 - 1 + 0.5D - p_180702_8_) / d3;
    ...
    protected boolean canReplaceBlock(IBlockState replacedFrom, IBlockState replacedTo)
    {
    	Block b = replacedFrom.getBlock();
    	return Util.isSoil(b) || Util.isGravel(b) || Util.isSand(b) || Util.isRawStone(b);
    }
    ...
    protected boolean isOceanBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ)
    {
        ...
        return block== FirmaMod.freshwater.getBlock() || block == FirmaMod.saltwater.getBlock();
    }
