...
public class FirmaGenRavine extends MapGenBase {
	protected static final IBlockState FLOWING_LAVA = FirmaMod.lava.getBlock().getDefaultState();
    ...
    protected boolean isOceanBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ)
    {
        ...
        return Util.isWater(block);
    }

    //Determine if the block at the specified location is the top block for the biome, we take into account
    //Vanilla bugs to make sure that we generate the map the same way vanilla does.
    private static boolean isTopBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ)
    {
    	return false;
        //net.minecraft.world.biome.Biome biome = world.getBiome(new BlockPos(x + chunkX * 16, 0, z + chunkZ * 16));
        //IBlockState state = data.getBlockState(x, y, z);
        //return (isExceptionBiome(biome) ? Util.isGrass(state.getBlock()) : state.getBlock() == biome.topBlock);
    }
