...
public class FirmaGenLakes extends WorldGenerator {
    private final Block block;

    public FirmaGenLakes(Block blockIn)
    {
        this.block = blockIn;
    }
		    ...
		    for (int j3 = 0; j3 < 16; ++j3)
		    {
		        for (int j4 = 4; j4 < 8; ++j4)
		        {
		            if (aboolean[(i2 * 16 + j3) * 8 + j4])
		            {
		                ...
		                if (Util.isDirt(worldIn.getBlockState(blockpos).getBlock()) && worldIn.getLightFor(EnumSkyBlock.SKY, position.add(i2, j4, j3)) > 0)
		                {
		                    //Biome biome = worldIn.getBiome(blockpos);

		                    worldIn.setBlockState(blockpos, Util.getGrass(worldIn.getBlockState(blockpos)), 2);
		                }
		            }
		        }
		    }
		    ...
		    for (int j2 = 0; j2 < 16; ++j2)
		    {
		        for (int k3 = 0; k3 < 16; ++k3)
		        {
		            for (int k4 = 0; k4 < 8; ++k4)
		            {
		                ...
		                {
		                    worldIn.setBlockState(position.add(j2, k4, k3), FirmaMod.rock.getStateFromMeta(RockEnum.Gabbro.getMeta()), 2);
		                }
		            }
		        }
		    }
		...
		if (this.block.getDefaultState().getMaterial() == Material.WATER)
		{
		    for (int k2 = 0; k2 < 16; ++k2)
		    {
		        for (int l3 = 0; l3 < 16; ++l3)
		        {
		            ...
		            {
		                worldIn.setBlockState(position.add(k2, 4, l3), FirmaMod.ice.getState(this.block), 2);
		            }
		        }
		    }
		}
