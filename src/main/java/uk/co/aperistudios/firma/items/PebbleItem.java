...
public class PebbleItem extends MetaItem {
	...
	public PebbleItem(String name) {
		super(name);
		setSubs(new String[] { "andesite", "basalt", "chalk", "chert", "claystone", "conglomerate", "dacite", "diorite",
				"dolomite", "gabbro", "gneiss", "granite", "limestone", "marble", "phyllite", "quartzite", "rhyolite",
				"rocksalt", "schist", "shale", "slate" });
	}
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer player, EnumHand handIn) {
		...
		if (player.getHeldItemMainhand().getCount() > 1) {
			if (worldIn.isRemote) {
				...
				//GuiKnapping.staticPlayerData = pd; // Client side only is fine
													// to keep its reference
				player.openGui(FirmaMod.instance, GuiHandler.GUI_KNAPPING, player.world, (int) player.posX,
